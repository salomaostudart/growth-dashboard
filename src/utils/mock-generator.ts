...
function generateForecast(
  data: number[],
  daysAhead: number,
): Array<{ date: string; projected: number; confidence: [number, number] }> {
  ...
  const now = new Date();
  const variance =
    recent.reduce((acc, v) => acc + (v - sumY / recent.length) ** 2, 0) / recent.length;
...
function generateDailyTraffic(
  days: number,
  baseSessions: number,
): Array<{ date: string; sessions: number; users: number }> {
...
export function generateSeoMetrics() {
  ...
  const topQueries = [
    ...
    positionHistory: Array.from({ length: 7 }, () =>
      Number(randomBetween(1 + i * 0.3, 8 + i * 1.5).toFixed(1)),
    ),
  }));
  ...
  return {
    ...
    pagePerformance: [
      {
        page: '/',
        impressions: randomInt(3000, 8000),
        clicks: randomInt(200, 600),
        ctr: Number(randomBetween(5, 12).toFixed(1)),
        position: Number(randomBetween(2, 6).toFixed(1)),
      },
      {
        page: '/platform',
        impressions: randomInt(1500, 4000),
        clicks: randomInt(100, 350),
        ctr: Number(randomBetween(4, 10).toFixed(1)),
        position: Number(randomBetween(3, 8).toFixed(1)),
      },
      {
        page: '/solutions',
        impressions: randomInt(1000, 3000),
        clicks: randomInt(50, 200),
        ctr: Number(randomBetween(3, 8).toFixed(1)),
        position: Number(randomBetween(5, 12).toFixed(1)),
      },
    ],
    targets: {
  ...
  for (let day = 0; day < 7; day++) {
    for (let hour = 6; hour < 22; hour++) {
      ...
      bestSendTimes.push({
        day,
        hour,
        engagement: Math.round(base + bonus + randomBetween(-10, 15)),
      });
    }
  }
    ...
    automations: [
      {
        ...
        successRate: Number(randomBetween(94, 99).toFixed(1)),
        totalRuns: randomInt(500, 2000),
      },
      {
        name: 'Lead Nurture',
        status: 'active' as const,
        lastTriggered: new Date().toISOString(),
        successRate: Number(randomBetween(90, 97).toFixed(1)),
        totalRuns: randomInt(200, 800),
      },
      {
        name: 'Re-engagement',
        status: 'active' as const,
        lastTriggered: new Date().toISOString(),
        successRate: Number(randomBetween(85, 95).toFixed(1)),
        totalRuns: randomInt(100, 400),
      },
      {
        name: 'Onboarding Flow',
        status: 'paused' as const,
        lastTriggered: new Date(Date.now() - 7 * 86400000).toISOString(),
        successRate: Number(randomBetween(88, 96).toFixed(1)),
        totalRuns: randomInt(50, 200),
      },
    ],
    bestSendTimes,
    targets: {
...
export function generateSocialMetrics() {
  ...
  return {
    platforms: [
      {
        name: 'LinkedIn',
        followers: randomInt(12000, 28000),
        followerGrowth: Number(randomBetween(2.5, 6.8).toFixed(1)),
        engagementRate: Number(randomBetween(3.2, 5.8).toFixed(1)),
        referralTraffic: randomInt(800, 2500),
        posts: randomInt(15, 30),
      },
      {
        name: 'Twitter/X',
        followers: randomInt(5000, 15000),
        followerGrowth: Number(randomBetween(1.0, 3.5).toFixed(1)),
        engagementRate: Number(randomBetween(1.5, 3.2).toFixed(1)),
        referralTraffic: randomInt(300, 1200),
        posts: randomInt(25, 60),
      },
      {
        name: 'YouTube',
        followers: randomInt(2000, 8000),
        followerGrowth: Number(randomBetween(3.0, 8.0).toFixed(1)),
        engagementRate: Number(randomBetween(4.0, 8.5).toFixed(1)),
        referralTraffic: randomInt(200, 800),
        posts: randomInt(4, 12),
      },
      {
        name: 'Instagram',
        followers: randomInt(3000, 10000),
        followerGrowth: Number(randomBetween(1.5, 4.2).toFixed(1)),
        engagementRate: Number(randomBetween(2.0, 4.5).toFixed(1)),
        referralTraffic: randomInt(100, 500),
        posts: randomInt(10, 25),
      },
    ],
    topPosts: Array.from({ length: 8 }, (_, i) => {
      ...
      return {
        ...
        content: [
          ...
          'New case study: 35% efficiency gains in manufacturing',
          'Watch: Platform demo walkthrough',
          'Behind the scenes: building our hybrid approach',
        ][i % 4],
        date: d.toISOString().split('T')[0],
    ...
    channelAttribution: [
      {
        ...
        leads: randomInt(200, 400),
        mql: randomInt(80, 180),
        sql: randomInt(25, 70),
        pipeline: randomInt(10, 35),
        won: randomInt(3, 12),
        cac: randomInt(50, 150),
      },
      {
        channel: 'Paid Search',
        leads: randomInt(150, 300),
        mql: randomInt(60, 130),
        sql: randomInt(20, 55),
        pipeline: randomInt(8, 25),
        won: randomInt(2, 8),
        cac: randomInt(180, 350),
      },
      {
        channel: 'Social Media',
        leads: randomInt(100, 250),
        mql: randomInt(40, 100),
        sql: randomInt(12, 35),
        pipeline: randomInt(5, 18),
        won: randomInt(1, 6),
        cac: randomInt(120, 280),
      },
      {
        channel: 'Email',
        leads: randomInt(80, 200),
        mql: randomInt(50, 120),
        sql: randomInt(18, 45),
        pipeline: randomInt(8, 22),
        won: randomInt(2, 8),
        cac: randomInt(30, 80),
      },
      {
        channel: 'Referral',
        leads: randomInt(50, 120),
        mql: randomInt(30, 70),
        sql: randomInt(15, 35),
        pipeline: randomInt(6, 18),
        won: randomInt(2, 7),
        cac: randomInt(20, 60),
      },
      {
        channel: 'Direct',
        leads: randomInt(80, 180),
        mql: randomInt(35, 80),
        sql: randomInt(10, 30),
        pipeline: randomInt(4, 15),
        won: randomInt(1, 5),
        cac: randomInt(0, 20),
      },
    ],
    pipelineByStage: [
      {
        ...
        count: randomInt(30, 60),
        confidence: 'low' as const,
      },
      {
        stage: 'Qualification',
        value: randomInt(150000, 400000),
        count: randomInt(20, 40),
        confidence: 'low' as const,
      },
      {
        stage: 'Proposal',
        value: randomInt(100000, 300000),
        count: randomInt(10, 25),
        confidence: 'medium' as const,
      },
      {
        stage: 'Negotiation',
        value: randomInt(80000, 200000),
        count: randomInt(5, 15),
        confidence: 'medium' as const,
      },
      {
        stage: 'Closed Won',
        value: randomInt(50000, 150000),
...
export function generateMartechHealth() {
  ...
  return {
    systems: [
      {
        ...
        uptime: Number(randomBetween(99.5, 99.99).toFixed(2)),
        lastSync: new Date(now.getTime() - randomInt(1, 30) * 60000).toISOString(),
        latencyMs: randomInt(80, 250),
        monthlyCost: 0,
      },
      {
        name: 'Google Search Console',
        status: 'healthy' as const,
        uptime: Number(randomBetween(99.0, 99.95).toFixed(2)),
        lastSync: new Date(now.getTime() - randomInt(60, 360) * 60000).toISOString(),
        latencyMs: randomInt(120, 400),
        monthlyCost: 0,
      },
      {
        name: 'HubSpot CRM',
        status: 'healthy' as const,
        uptime: Number(randomBetween(99.5, 99.99).toFixed(2)),
        lastSync: new Date(now.getTime() - randomInt(1, 15) * 60000).toISOString(),
        latencyMs: randomInt(150, 350),
        monthlyCost: 800,
      },
      {
        name: 'Webflow CMS',
        status: 'healthy' as const,
        uptime: Number(randomBetween(99.8, 99.99).toFixed(2)),
        lastSync: new Date(now.getTime() - randomInt(30, 120) * 60000).toISOString(),
        latencyMs: randomInt(50, 150),
        monthlyCost: 39,
      },
      {
        name: 'Mailchimp',
        status: 'degraded' as const,
        uptime: Number(randomBetween(97.0, 99.5).toFixed(2)),
        lastSync: new Date(now.getTime() - randomInt(120, 480) * 60000).toISOString(),
        latencyMs: randomInt(300, 800),
        monthlyCost: 150,
      },
      {
        name: 'LinkedIn API',
        status: 'healthy' as const,
        uptime: Number(randomBetween(98.5, 99.8).toFixed(2)),
        lastSync: new Date(now.getTime() - randomInt(30, 180) * 60000).toISOString(),
        latencyMs: randomInt(200, 500),
        monthlyCost: 0,
      },
    ],
    automationLog: Array.from({ length: 20 }, (_, i) => {
      ...
      const statuses: Array<'success' | 'failed' | 'partial'> = [
        'success',
        'success',
        'success',
        'success',
