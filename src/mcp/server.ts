...
import { formatCurrency, formatLatency, formatNumber, formatPercent } from '../utils/formatters.js';
  ...
  async () => {
    ...
    if (web.bounceRate > 50) {
      alerts.push({
        severity: 'warning',
        source: 'GA4',
        message: `Bounce rate is ${web.bounceRate.toFixed(1)}% (threshold: 50%)`,
      });
    }
    ...
    martech.systems
      ...
      .forEach((s) => {
        alerts.push({
          severity: 'danger',
          source: 'Martech',
          message: `${s.name} is ${s.status} (uptime: ${s.uptime.toFixed(2)}%)`,
        });
      });
    ...
    if (failedAutos.length > 0) {
      alerts.push({
        severity: 'warning',
        source: 'Martech',
        message: `${failedAutos.length} failed automation(s) in recent log`,
      });
    }
    ...
    const summary = {
      ...
      pipeline: {
        ...
        winRate: formatPercent(
          crm.funnel.pipeline > 0 ? (crm.funnel.won / crm.funnel.pipeline) * 100 : 0,
        ),
      },
...
server.tool(
  ...
  'Deactivate all triggered alerts, or a specific metric alert',
  {
    metric: z
      .string()
      .optional()
      .describe('Optional: clear only alerts for this metric. Omit to clear all.'),
  },
  async ({ metric }) => {
    const alerts = loadAlerts();
    let cleared = 0;
...
server.tool(
  ...
  {
    action: z
      .enum(['list', 'describe_role'])
      .describe('"list" to show all roles, "describe_role" to show permissions for a role'),
    role: z
      .enum(['admin', 'analyst', 'viewer'])
      .optional()
      .describe('Role to describe (for describe_role action)'),
  },
  async ({ action, role }) => {
          ...
          text: JSON.stringify(
            {
              roles: [
                {
                  role: 'admin',
                  description: 'Full access — manage users, alerts, audit log, all data',
                },
                {
                  role: 'analyst',
                  description: 'Data access — view revenue, manage alerts, use chat, export',
                },
